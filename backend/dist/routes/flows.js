...
router.get('/:id', async (req, res) => {
    try {
        ...
        const flow = await database_1.default.flow.findUnique({
            where: { id: req.params.id },
        });
        if (!(await (0, types_1.verifyOwnership)(flow, req.user.userId, res, 'Fluxo')))
            return;
        res.json(flow);
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const flow = await database_1.default.flow.create({
            data: {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existing = await database_1.default.flow.findUnique({ where: { id: req.params.id } });
        if (!(await (0, types_1.verifyOwnership)(existing, req.user.userId, res, 'Fluxo')))
            return;
        const { name, description, nodes, edges, isActive, triggerType, triggerValue } = req.body;
        const flow = await database_1.default.flow.update({
            where: { id: req.params.id },
            data: {
                name,
                description,
...
router.put('/:id/toggle', async (req, res) => {
    try {
        ...
        const flow = await database_1.default.flow.findUnique({ where: { id: req.params.id } });
        const ownedFlow = await (0, types_1.verifyOwnership)(flow, req.user.userId, res, 'Fluxo');
        if (!ownedFlow)
            return;
        const updated = await database_1.default.flow.update({
            where: { id: req.params.id },
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (!(await (0, types_1.verifyOwnership)(existing, req.user.userId, res, 'Fluxo')))
            return;
        await database_1.default.flow.delete({ where: { id: req.params.id } });
...
router.post('/:id/duplicate', async (req, res) => {
    try {
        ...
        const original = await database_1.default.flow.findUnique({ where: { id: req.params.id } });
        const ownedOriginal = await (0, types_1.verifyOwnership)(original, req.user.userId, res, 'Fluxo');
        if (!ownedOriginal)
            return;
        const duplicate = await database_1.default.flow.create({
            data: {
