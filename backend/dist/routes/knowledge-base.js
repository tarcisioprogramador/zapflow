...
router.get('/', async (req, res) => {
    try {
        ...
        if (search) {
            ...
            const items = await database_1.default.knowledgeBaseItem.findMany({ where: { organizationId: user.organizationId } });
            const filtered = items.filter((i) => i.title.toLowerCase().includes(searchStr) || i.content.toLowerCase().includes(searchStr));
            res.json(filtered);
            return;
        }
        const items = await database_1.default.knowledgeBaseItem.findMany({ where, orderBy: { updatedAt: 'desc' } });
        res.json(items);
    }
    catch (error) {
...
router.get('/stats', async (req, res) => {
    try {
        const user = await database_1.default.user.findUnique({ where: { id: req.user.userId } });
        if (!user?.organizationId) {
            res.json({ total: 0, categories: 0, characters: 0 });
            return;
        }
        const items = await database_1.default.knowledgeBaseItem.findMany({
...
router.post('/', async (req, res) => {
    try {
        ...
        const item = await database_1.default.knowledgeBaseItem.create({
...
router.post('/ai-context', async (req, res) => {
    try {
        ...
        items.forEach((item) => {
            if (!contextByCategory[item.category])
                contextByCategory[item.category] = [];
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { title, content, category } = req.body;
        const item = await database_1.default.knowledgeBaseItem.update({
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const item = await database_1.default.knowledgeBaseItem.findUnique({ where: { id: req.params.id } });
        if (!item || item.organizationId !== user.organizationId) {
            res.status(404).json({ error: 'Item não encontrado' });
            return;
        }
        await database_1.default.knowledgeBaseItem.delete({ where: { id: req.params.id } });
